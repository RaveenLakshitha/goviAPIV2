const Category = require("../../models/Categories/infoCategory");
  ...
  try {
    if (req.user.role === "Admin") {
      ...
      if (!created) {
        ...
        if (categoryObj.parentId) {
          var Info = await information.create({
            Title: cat.categoryName,
            categoryId: cat._id,
          });
          await infoCategory.findByIdAndUpdate(Info.categoryId, {
            $push: {
              Information: Info.id,
            },
          });
...
exports.hideCategory = async (req, res, next) => {
  try {
    ...
    if (!category) {
      return next(
        new ErrorResponse(
          `Category not Found With name of ${req.params.categoryName}`,
          404
        )
      );
    }
