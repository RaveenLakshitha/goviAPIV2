...
exports.getAppointmentSlotsByExpert = async (req, res, next) => {
  try {
    ...
    if (!slot) {
      return next(
        new ErrorResponse(
          `Slots not Found With Expert id of ${req.params.id}`,
          404
        )
      );
    }
...
exports.createAppointmentSlot = async (req, res, next) => {
  try {
    const profile = await expert.findOne({ user: req.user.id });
    if (!profile) {
      return next(
        new ErrorResponse(
    ...
    req.body.expertId = req.user.id;
    const slot = await Slot.create(req.body);
    await expert.findOneAndUpdate(
      { expert: req.user.expertID },
      {
        $push: {
...
exports.deleteAppointmentSlot = async (req, res, next) => {
  try {
    ...
    await expert.findByIdAndUpdate(req.user.expertId, {
      $pull: { appointmentSlots: slot.id },
    });
